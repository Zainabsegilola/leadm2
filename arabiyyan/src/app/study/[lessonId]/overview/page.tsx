  ...
  function renderArabicWords(arabic: string) {
    return arabic.split(" ").map((word, i) => (
      <button
        ...
        className="font-arabic px-0.5 transition-colors"
        ...
        <div className="mx-auto max-w-3xl">
          ...
          {page === 1 && (
            <div className="mx-auto max-w-xl space-y-5">
              ...
              <p className="pt-4 text-xs" style={{ color: "var(--muted-dim)" }}>
                Read it once or twice. You don&rsquo;t need to memorise anything yet.
              </p>
            </div>
          )}
          ...
          {page === 2 && (
            <div className="space-y-10">
              ...
              <p className="text-center text-xs" style={{ color: "var(--muted-dim)" }}>
                Hover a sentence to see its match on the other side. Click the Arabic to hear it.
              </p>
          ...
          {page === 3 && (
            <div className="mx-auto max-w-2xl space-y-10">
              <div className="space-y-6" dir="rtl">
                {lesson.sentences.map((s) => (
                  <p
                    key={s.id}
                    onClick={() => audioRef.current?.play(s.id)}
                    className="font-arabic cursor-pointer text-3xl leading-loose"
                  >
                    {renderArabicWords(s.arabic)}
                  </p>
                ))}
              ...
              <p className="text-center text-xs" style={{ color: "var(--muted-dim)" }}>
                Tap a sentence to hear it. Tap a word to mark it red — &ldquo;I don&rsquo;t know this yet.&rdquo;
              </p>
              <div className="flex justify-center">
                <PassageAudioPlayer ref={audioRef} items={audioItems} />
              </div>
