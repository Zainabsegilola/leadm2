...
export function GlossGrid({
  ...
  return (
    <div className="space-y-8">
      ...
      <div dir="rtl" className="flex flex-wrap justify-center gap-x-5 gap-y-4">
        {tokens.map((token, i) => (
          <div key={i} className="flex flex-col items-center gap-1.5">
            <span className="text-sm sm:text-base leading-tight" style={{ color: "var(--foreground)" }} dir="ltr">
              {token.gloss}
            </span>
            {recall ? (
              <span className="font-arabic text-xl sm:text-2xl tracking-wide" style={{ color: "var(--muted)" }}>
                {token.arabic.trim()[0]}
              </span>
            ) : (
              <WordPopover
      ...
      {/* Row 4: complete Arabic sentence, centred, RTL */}
      {recall && !revealed ? (
        ...
        <p className="font-arabic text-3xl sm:text-4xl text-center leading-loose" dir="rtl">
          {arabicSentence}
        </p>
      )}
    </div>
