...
`src/app/api/subscribe/route.ts` is already wired up to **Kit** (formerly ConvertKit) — it just needs your credentials.

1. Sign up at kit.com (free up to 10,000 subscribers) and create a form to collect these emails into.
2. In Kit: **Settings -> Developer -> API Keys -> V4 keys -> Add a new key**. Copy it.
3. Find your form's ID (visible in the form's URL/settings in Kit, or via the API's list-forms endpoint).
4. Copy `.env.local.example` to `.env.local` and fill in `KIT_API_KEY` and `KIT_FORM_ID` for local testing.
5. In Vercel: **Project Settings -> Environment Variables**, add the same two variables, then redeploy.

Until those two variables are set, the route just logs the email to the server console instead of failing — so local dev works before you've connected anything.

Using a different provider (Mailchimp, Beehiiv, etc.) instead? The shape stays the same — swap the `fetch` call inside `route.ts` for that provider's API, and swap the two env var names to match.
