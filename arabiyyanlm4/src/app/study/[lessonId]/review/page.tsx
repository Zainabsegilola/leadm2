...
export default function ReviewPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  const forecastValues = progress
    ? buildDueForecast(lesson.sentences.map((s) => progress.review[s.id]), forecastDays)
    : new Array(forecastDays).fill(0);
  const forecastLabels = buildForecastLabels(forecastDays);
          ...
          {allProgrammesComplete ? (
            ...
            <>
              ...
              <div className="mb-10">
                <div className="mb-2 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px]" style={{ color: "var(--muted-dim)" }}>
                      How many sentences are scheduled for review each day.
                    </p>
                  </div>
                  <div className="flex gap-1 text-[11px]">
                    {([7, 14] as const).map((d) => (
                      <button
                        ...
                        className="interactive rounded-full px-2.5 py-1"
                        style={{
                          background: forecastDays === d ? "var(--emerald-dim)" : "transparent",
                          color: forecastDays === d ? "var(--emerald-glow)" : "var(--muted-dim)",
                        }}
                      >
                        {d}d
                      </button>
                ...
                <DueForecastChart labels={forecastLabels} values={forecastValues} />
              </div>
